...
\title{Bayes Rules. Bayesian Hierarchical Cluster Zero-Inflation Models}
...
Furthermore, we will be operating in a setting where the data is not only zero-inflated, but also exhibits a clustered structure. Again, the notion of data being clustered is a very non precise one. For this thesis we will not put a very strong restriction on what this looks like. Our data will be clustered in the sense that there is meaningful grouping in the data structure that makes data points within the same cluster more alike on average than points across clusters.
